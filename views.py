...
def cadastro(request):
    ...
    messages.success(request, 'Registrado com sucesso!Utilize os campos abaixo '
                              'para fazer login.')
...
def dashboard(request):
    ...
    if not form.is_valid():
        messages.error('Erro ao Enviar fomulário.')
    ...
    if len(descricao)<5:
        messages.error('Descição precisa ter mais de 5 caracteres.')
        form = FormContato(request.POST)
        return render(request, 'accounts/dashboard.html', {'form': form})
